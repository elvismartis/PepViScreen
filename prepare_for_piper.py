...
def filter_missing_backbone(pdb_in, pdb_out):
    ...
    for line in lines:
        if line.startswith('ATOM  '):
            ...
            atoms.append({'atom_i': atom_i, 'chain_id': chain_id, 'residue_id': residue_id,
							'residue_name': residue_name, 'atom_name': atom_name, 'line': line })
    ...
    for residue in pdb_residue_list:
        pdb_residues.append( list(filter(lambda x: x['chain_id'] == residue[0] and
									x['residue_id'] == residue[2] and x['residue_name'] == residue[1], atoms ) ))
    ...
    with open(out_pdb, 'w') as out_f:
        for residue in pdb_residues:
            ...
            if residue_name not in resis:
                print ("Residue %s not supported, stripped for mapping" % residue_name, file=sys.stderr)
                continue
...
def pdbpqr(base_dir, pdb, chains):
    ...
    minlog       = pdb_prefix+"_pmin.log";


    chains = [x.upper() for x in chains]

    if len(chains) == 0:
        add_chain(pdb, genout)
    else:
    ...
    with open('error_log', 'a') as err_out, open('output_log', 'a') as out_f:
        subprocess.call(['pdb2pqr', '--nodebump', '--noopt', '--ff=AMBER',
							'--ffout=AMBER', genout, pqr],
							stdout=out_f, stderr=err_out)
    pdb_norm(genout, nonminout)
